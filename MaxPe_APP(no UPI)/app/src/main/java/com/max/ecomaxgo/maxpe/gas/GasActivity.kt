package com.max.ecomaxgo.maxpe.gas
...
class GasActivity : AppCompatActivity() {
    ...
    private fun getOperator(){
        val call: Call<Bbps> =
            retrofitconfig.endpoints.getDthOperator(Constant.skey, "GAS")
        call.enqueue(object : Callback<Bbps> {
            override fun onResponse(call: Call<Bbps>, response: Response<Bbps>) {
                ...
                if (bbpsModel.status.equals("1")){
                    ...
                    bbpsAdapter = BbpsGetOperatorAdapter(this@GasActivity, operaterDetails)
